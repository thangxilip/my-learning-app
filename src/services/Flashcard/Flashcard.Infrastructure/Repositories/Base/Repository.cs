...
public class Repository<T>(FlashcardDbContext context) : IRepository<T>
    where T : BaseEntity
{
    protected FlashcardDbContext Context { get; } = context;
    ...
    {
        return DbSet.AsNoTracking().AnyAsync(x => x.Id == id, cancellationToken);
    }
