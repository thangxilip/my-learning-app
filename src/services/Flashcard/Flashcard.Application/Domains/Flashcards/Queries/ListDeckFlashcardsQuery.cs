...
public record ListDeckFlashcardsQuery(Guid UserId, Guid DeckId) : IRequest<List<FlashcardDto>>;

public class ListDeckFlashcardsQueryHandler(IUnitOfWork unitOfWork)
    : IRequestHandler<ListDeckFlashcardsQuery, List<FlashcardDto>>
{
    public async Task<List<FlashcardDto>> Handle(ListDeckFlashcardsQuery request, CancellationToken cancellationToken)
    {
